...
def main():
    ...
    if option == "Add Emoji To Text":
        ...
        if submit:
            st.write(text)
            update_database(text, 'U+1F600')
            with tab1:
                st.write("Show results generated by trained model")
            with tab2:
...
def update_database(text, emojis):
    vocab = text.split()
    emoji = [0,1,1,0,0] # assume there's some function that returned a emoji encoder
    for v in vocab:
